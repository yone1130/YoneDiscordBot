...
class BotDatabase:
    ...
    def get_rank(self, *, user: discord.User.id) -> list:
        ...
        db_cur = self.cursor(connect=db_con)
        db_cur.execute(
            f"SELECT uid, level, totalPoint, point, msgCnt, ltrCnt FROM rank WHERE uid='{user}'"
        )
    ...
    def update_rank(
        ...
        *,
        ...
        db_cur = self.cursor(connect=db_con)
        db_cur.execute(
            f"UPDATE rank SET level='{level}', totalPoint='{total_point}', point='{point}', msgCnt='{msg_cnt}', ltrCnt='{letter_cnt}' WHERE uid='{str(user)}'"
        )
        self.save(connect=db_con)
